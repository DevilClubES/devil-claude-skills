...
2. Classify existing work as `extension`, `preserve`, or `overhaul` before editing. Protect routes, information architecture, analytics, forms, accessibility behavior, and recognizable product identity unless the requested mode authorizes changing them.
3. Match the user's requested operation. Review-only requests do not authorize edits; build, redesign, refine, tweak, and ship requests do.
4. Reuse shipped components and semantic tokens before inventing new ones. Label any new token or pattern as proposed until implemented and verified.
5. Keep concept artifacts distinct from production implementation. Never describe an unrendered draft as shipped or verified.
6. Use supplied facts and assets. Do not fabricate metrics, testimonials, customer logos, research findings, prices, or product capabilities to fill a layout.
7. Preserve principles from references without cloning protected logos, copy, illustrations, proprietary assets, or another product's recognizable trade dress.
8. Verify visual work in the rendered surface at relevant breakpoints and states. Source review alone is insufficient for a visual completion claim.
9. Prefer a small coherent system over a large catalog of disconnected styles. Novelty must serve the brief.
10. Ask only for information whose absence would materially change the result. When the user delegates judgment, choose a defensible default and record it.
11. Keep user-facing updates concise. Put durable decisions in the project, not only in chat.
...
Read [design-read.md](references/design-read.md) completely for a net-new build, extension, or redesign. Record the work mode and calibrate variance, motion, density, asset dependence, and brand fidelity. These dials are decisions that must affect the artifact, not decorative scores.

When external pages, screenshots, moodboards, or products influence the direction, read [reference-ledger.md](references/reference-ledger.md) completely and maintain a source ledger before implementing. Extract transferable mechanisms while replacing source-specific staging and signature elements.
...
Build an early v0 only when it resolves a costly uncertainty about macrostructure, density, assets, or interaction. Keep it disposable and representative of the hardest section. Do not polish a v0, mistake it for the selected direction, or force it into straightforward work.
...
- For purposeful interface motion, apply the motion checks in `../design-critique/references/motion-craft.md` before adding animation.
...
- Reference-ledger decisions and originality caveats when external work influenced the result
...
The five-dial Design Read, preservation-aware redesign classification, and conditional early-v0 checkpoint are independently adapted from `ConardLi/garden-skills` at commit `aaf9a82f5efd`. The source-ledger and mechanism-versus-staging checks are independently adapted from `MengTo/Skills` at commit `4c716b516b6b`. No source assets, demos, layouts, or visual systems are vendored.
