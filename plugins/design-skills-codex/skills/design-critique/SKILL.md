...
7. When motion is visible or proposed, read [motion-craft.md](references/motion-craft.md) completely and inspect purpose, frequency, interruption behavior, performance, and reduced-motion parity.
8. When the artifact is reference-led, read [originality-check.md](references/originality-check.md) completely. Report overlap and originality risk without making unsupported legal conclusions.
...
For motion-bearing work, also report the interaction reviewed, trigger frequency, interruption behavior, reduced-motion result, and whether slow-motion inspection exposed an easing or continuity defect. For reference-led work, include the source ledger and any signature-element overlap that still needs replacement.
...
The motion rubric is independently adapted from `emilkowalski/skills` at commit `d23d7f88a2e2`. The reference-led originality rubric is independently adapted from `MengTo/Skills` at commit `4c716b516b6b` and selected anti-cliche checks from `codeswithroh/tastemaker` at commit `45313ce9f609`. No source code, demo, asset, or visual composition is vendored.
