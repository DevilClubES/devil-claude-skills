...
When exact foreground/background values are available, verify declared pairings with the shared deterministic utility before recording them as accessible:

```text
node "<design-skills-plugin-directory>/scripts/check-contrast.mjs" <foreground> <background> [minimum]
node "<design-skills-plugin-directory>/scripts/check-contrast.mjs" --matrix <pairs.json>
```

The utility calculates opaque sRGB contrast. It does not replace rendered checks for transparency, gradients, images, states, or font size/weight classification.
